...
function openFromFrame (e, url) {
    ...
    .then((response) => response.text())
    .then((html) => {
        parent.document.getElementsByClassName("wrapper")[0].innerHTML = html;
    })
    .catch((error) => {
        console.warn(error);
    });

    parent.document.getElementsByClassName("footer")[0].style.opacity = "1";
    parent.document.getElementsByClassName("footer")[0].style.pointerEvents = "all";
